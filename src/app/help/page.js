      ...
      <div className="mx-auto max-w-3xl px-5 py-10 pb-16">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8 flex items-start justify-between gap-4 flex-wrap">
          <div>
            ...
            <p className="mt-1 text-sm text-[var(--text-muted)]">Submit a query and we'll get back to you as soon as possible!</p>
          </div>
          <motion.button
            whileHover={{ scale: 1.02 }}
            whileTap={{ scale: 0.98 }}
            onClick={() => setShowForm(!showForm)}
            className="rounded-full bg-[var(--btn-dark)] px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-[var(--btn-dark-hover)] flex-shrink-0"
          >
              ...
              <div className="rounded-[1.4rem] bg-white shadow-lg shadow-[var(--shadow)]/5 p-6">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <motion.button
                    ...
                    whileTap={{ scale: 0.98 }}
                    disabled={submitting}
                    className="w-full rounded-full bg-[var(--btn-dark)] py-3.5 text-sm font-semibold text-white transition hover:bg-[var(--btn-dark-hover)]"
                  >
        ...
        {/* Tickets List */}
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="rounded-[1.4rem] bg-white shadow-lg shadow-[var(--shadow)]/5 p-6">
          ...
          {loading ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-sm text-[var(--text-muted)] mb-4">No queries yet!</p>
              <button
                onClick={() => setShowForm(true)}
                className="rounded-full bg-[var(--btn-dark)] px-6 py-2.5 text-sm font-semibold text-white transition hover:bg-[var(--btn-dark-hover)]"
              >
            ...
            <div className="space-y-4">
              {tickets.map(ticket => {
                ...
                return (
                  <div key={ticket.id} className="rounded-2xl border border-[var(--border)]/10 bg-[var(--bg)] overflow-hidden">
                    {/* Ticket Header */}
                    <button
                    ...
                    >
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-[var(--text-placeholder)] mt-0.5">
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0">
                        ...
                        <span className="text-[var(--text-placeholder)] text-sm">{isExpanded ? '▲' : '▼'}</span>
                      </div>
                    </button>
                    ...
                    <AnimatePresence>
                      {isExpanded && (
                        <motion.div
                          ...
                          className="overflow-hidden border-t border-[var(--border)]/10"
                        >
                          {/* Messages Thread */}
                          <div className="p-4 space-y-3 max-h-80 overflow-y-auto">
                            {messages.length === 0 ? (
                              <p className="text-sm text-[var(--text-placeholder)] text-center py-4">No messages yet</p>
                            ) : (
                              messages.map((msg, i) => (
                                <div
                                  key={i}
                                ...
                                >
                                  <div className={`max-w-[80%] rounded-2xl px-4 py-3 ${
                                    msg.sender === 'user'
                                      ? 'bg-[var(--btn-dark)] text-white rounded-br-sm'
                                      : 'bg-white border border-[var(--border)]/10 text-[var(--text-primary)] rounded-bl-sm'
                                  }`}>
                                    <p className={`text-xs font-semibold mb-1 ${msg.sender === 'user' ? 'text-white/60' : 'text-[var(--accent)]'}`}>
                                      {msg.sender === 'user' ? 'You' : 'Support Team'}
                                    </p>
                                    <p className="text-sm leading-relaxed">{msg.text}</p>
                                    <p className={`text-xs mt-1 ${msg.sender === 'user' ? 'text-white/40' : 'text-[var(--text-placeholder)]'}`}>
                          ...
                          {ticket.status !== 'resolved' ? (
                            <div className="p-4 border-t border-[var(--border)]/10 bg-white">
                              <div className="flex gap-3">
                                <textarea
                                  ...
                                  rows={2}
                                  className="flex-1 rounded-2xl border border-[var(--border)]/15 bg-[var(--bg)] px-4 py-3 text-sm text-[var(--text-primary)] placeholder-[var(--text-placeholder)] focus:outline-none focus:border-[var(--border)]/30 transition resize-none"
                                  onKeyDown={(e) => {
                                ...
                                <motion.button
                                  ...
                                  className="flex-shrink-0 rounded-full bg-[var(--btn-dark)] px-5 py-2 text-sm font-semibold text-white transition hover:bg-[var(--btn-dark-hover)] disabled:opacity-50 disabled:cursor-not-allowed self-end"
                                >
                                  {sendingReply === ticket.id ? '...' : 'Send'}
                                </motion.button>
                              </div>
                              <p className="text-xs text-[var(--text-placeholder)] mt-2">Press Enter to send, Shift+Enter for new line</p>
                            </div>
                          ) : (
                            <div className="p-4 border-t border-[var(--border)]/10 bg-green-50 text-center">
