    ...
    <main className="min-h-screen bg-[var(--bg)] text-[var(--text-primary)]">
      ...
      <div className="mx-auto max-w-4xl px-5 py-8 pb-16 space-y-4">
        ...
        {loading ? (
          ...
          <div className="text-center py-20 rounded-[2rem] border border-[var(--border)]/10 bg-[var(--bg-card)]/55"><p className="text-4xl mb-4">🎧</p><p className="text-[var(--text-muted)]">No tickets yet!</p></div>
        ) : (
          <div className="space-y-4">
            {tickets.map((ticket) => {
              ...
              return (
                <div key={ticket.id} className="rounded-[1.4rem] bg-white shadow-lg shadow-[var(--shadow)]/5 overflow-hidden">
                  <button onClick={() => setExpandedTicket(isExpanded ? null : ticket.id)} className="w-full flex items-center justify-between gap-4 p-5 text-left hover:bg-[#f9f6f2] transition">
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-sm text-[var(--text-muted)] mt-0.5">{ticket.name} · {ticket.email}</p>
                      <p className="text-xs text-[var(--text-placeholder)] mt-0.5">{messages.length} messages</p>
                    </div>
                  ...
                  <AnimatePresence>
                    {isExpanded && (
                      <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden border-t border-[var(--border)]/10">
                        <div className="p-4 space-y-3 max-h-72 overflow-y-auto bg-[var(--bg)]">
                          {messages.map((msg, j) => (
                            <div key={j} className={`flex ${msg.sender === 'admin' ? 'justify-end' : 'justify-start'}`}>
                              <div className={`max-w-[80%] rounded-2xl px-4 py-3 ${msg.sender === 'admin' ? 'bg-[var(--btn-dark)] text-white rounded-br-sm' : 'bg-white border border-[var(--border)]/10 text-[var(--text-primary)] rounded-bl-sm'}`}>
                                <p className={`text-xs font-semibold mb-1 ${msg.sender === 'admin' ? 'text-white/60' : 'text-[var(--accent)]'}`}>{msg.sender === 'admin' ? 'Support Team' : ticket.name}</p>
                          ...
                          <div className="p-4 border-t border-[var(--border)]/10 bg-white">
                            <div className="flex gap-2">
                              <textarea
                                ...
                                rows={2}
                                className="flex-1 rounded-2xl border border-[var(--border)]/15 bg-[var(--bg)] px-4 py-2.5 text-sm placeholder-[var(--text-placeholder)] focus:outline-none transition resize-none"
                                onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleReply(ticket.id) } }}
                              />
                              <div className="flex flex-col gap-2">
                                <button onClick={() => handleReply(ticket.id)} disabled={sendingReply === ticket.id || !replyForm[ticket.id]?.trim()} className="rounded-full bg-[var(--btn-dark)] px-4 py-2 text-xs font-semibold text-white disabled:opacity-50">
                        ...
                        {!permissions.includes('reply_tickets') && (
                          <div className="p-4 border-t border-[var(--border)]/10 bg-[var(--bg)] text-center">
                            <p className="text-xs text-[var(--text-placeholder)] italic">View only — no permission to reply</p>
                          </div>
                        )}
                      </motion.div>
                    )}
