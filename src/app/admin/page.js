...
export default function AdminDashboard() {
  ...
  const fetchProducts = () => { fetch('/api/products').then(r => r.json()).then(d => setProducts(d.products || [])) }
  const fetchTickets = () => { fetch('/api/admin/tickets').then(r => r.json()).then(d => setTickets(d.tickets || [])) }
  const fetchCoupons = () => { fetch('/api/admin/coupons').then(r => r.json()).then(d => setCoupons(d.coupons || [])) }
  const fetchAdmins = () => { fetch('/api/admin/admins').then(r => r.json()).then(d => setAdmins(d.admins || [])) }
  ...
  const handleCancellation = async (id, action) => {
    ...
    setUpdating(null); fetchCancellations(); fetchOrders()
  }
  ...
  const addPincode = (pincodes, setPincodes, input, setInput) => {
    ...
    setPincodes([...pincodes, trimmed]); setInput('')
  }

  const handleAddProduct = async (e) => {
    e.preventDefault(); setSubmittingProduct(true)
    const res = await fetch('/api/admin/products', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...productForm, images: productForm.images.filter(u => u.trim()), price: parseFloat(productForm.price), originalPrice: parseFloat(productForm.originalPrice), stock: parseInt(productForm.stock) }) })
    const data = await res.json(); setProductMessage(data.message); setSubmittingProduct(false)
    if (res.ok) { setProductForm({ name: '', description: '', price: '', originalPrice: '', images: [''], category: 'Electronics', stock: '', featured: false, sameDayPincodes: [] }); setPincodeInput(''); fetchProducts() }
  }

  const handleUpdateProduct = async () => {
    setUpdating('product')
    await fetch('/api/admin/products/' + editProduct.id, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...editForm, images: editForm.images.filter(u => u.trim()), price: parseFloat(editForm.price), originalPrice: parseFloat(editForm.originalPrice), stock: parseInt(editForm.stock) }) })
    setUpdating(null); setEditProduct(null); setEditForm(null); fetchProducts()
  }
  ...
  const toggleFeatured = async (product) => {
    await fetch('/api/admin/products/' + product.id, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...product, featured: !product.featured }) }); fetchProducts()
  }

  const handleTicketReply = async (ticketId) => {
    const text = replyForm[ticketId]; if (!text?.trim()) return
    ...
    setSendingReply(null); setReplyForm(prev => ({ ...prev, [ticketId]: '' })); fetchTickets()
  }

  const handleTicketResolve = async (id, status) => {
    await fetch('/api/admin/tickets/' + id, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ status }) }); fetchTickets()
  }
  ...
  const fetchUserOrders = async (email) => {
    ...
    setUserOrders(prev => ({ ...prev, [email]: data.orders || [] })); setExpandedUser(email)
  }
  ...
  const handleCouponSubmit = async (e) => {
    ...
    const data = await res.json(); setCouponMessage(data.message); setSubmittingCoupon(false)
  ...
  const handleDeleteCoupon = async (id) => {
    ...
    await fetch('/api/admin/coupons/' + id, { method: 'DELETE' }); fetchCoupons()
  }

  const handleAdminSubmit = async (e) => {
    e.preventDefault(); setSubmittingAdmin(true)
    const url = editAdmin ? '/api/admin/admins/' + editAdmin.id : '/api/admin/admins'
    const method = editAdmin ? 'PATCH' : 'POST'
    const body = { ...adminForm }; if (editAdmin && !body.password) delete body.password
    const res = await fetch(url, { method, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) })
    const data = await res.json(); setAdminMessage(data.message); setSubmittingAdmin(false)
  ...
  const handleDeleteAdmin = async (id) => {
    ...
    await fetch('/api/admin/admins/' + id, { method: 'DELETE' }); fetchAdmins()
  }
  ...
  const handleSaveSettings = async () => {
    ...
    const data = await res.json(); setSavingSettings(false); setSettingsMessage(data.message)
    setTimeout(() => setSettingsMessage(''), 3000)
  }

  const handleAddCategory = async (e) => {
    e.preventDefault(); if (!categoryForm.name.trim()) return
    setSubmittingCategory(true); setCategoryMessage('')
    ...
    const data = await res.json(); setSubmittingCategory(false); setCategoryMessage(data.message)
    if (res.ok) { setCategoryForm({ name: '', icon: '🛍️', image: '' }); fetchCategories(); setTimeout(() => setCategoryMessage(''), 3000) }
  }

  const handleToggleCategory = async (category) => {
    await fetch('/api/admin/categories/' + category.id, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...category, isActive: !category.isActive }) }); fetchCategories()
  }

  const handleDeleteCategory = async (id) => {
    if (!confirm('Delete this category? Make sure no products use it.')) return
    const res = await fetch('/api/admin/categories/' + id, { method: 'DELETE' })
    const data = await res.json(); if (!res.ok) { alert(data.message); return }; fetchCategories()
  }

  const handleUpdateCategory = async (category) => {
    await fetch('/api/admin/categories/' + category.id, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: editCategory.name, icon: editCategory.icon, image: editCategory.image || '', isActive: category.isActive, sortOrder: category.sortOrder }) })
    setEditCategory(null); fetchCategories()
  }
    ...
    <div className="min-h-screen bg-[#f6f1ea] text-[#171313] flex [&_input]:text-[#171313] [&_textarea]:text-[#171313] [&_select]:text-[#171313] [&_option]:text-[#171313] [&_input]:placeholder:text-[#9b8f86] [&_textarea]:placeholder:text-[#9b8f86]">
      ...
      <aside className={`fixed left-0 top-0 h-full bg-[#171313] z-40 transition-all duration-300 flex flex-col ${sidebarOpen ? 'w-56' : 'w-16'}`}>
        <div className="p-4 border-b border-white/10 flex items-center gap-3">
          {config.logo ? <img src={config.logo} alt={config.brandName} className="w-8 h-8 rounded-full object-cover flex-shrink-0"/> : <div className="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center text-white text-xs font-bold flex-shrink-0">{config.shortCode}</div>}
        ...
        <nav className="flex-1 p-2 space-y-1 overflow-y-auto">
          {TABS.map(tab => {
            ...
            return (
              <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition text-left ${activeTab === tab.id ? 'bg-white/15 text-white' : 'text-white/50 hover:bg-white/8 hover:text-white'}`}>
                <span className="text-lg flex-shrink-0">{tab.icon}</span>
                {sidebarOpen && <span className="text-sm font-medium flex-1 truncate">{tab.label}</span>}
                {badge > 0 && sidebarOpen && <span className="bg-red-500 text-white text-xs font-bold px-1.5 py-0.5 rounded-full flex-shrink-0">{badge}</span>}
                {badge > 0 && !sidebarOpen && <span className="absolute left-8 top-0 bg-red-500 text-white text-xs font-bold w-4 h-4 rounded-full flex items-center justify-center">{badge}</span>}
              </button>
            )
          })}
        </nav>
        <div className="p-2 border-t border-white/10 space-y-1">
          ...
          <button onClick={async () => { await fetch('/api/admin/logout', { method: 'POST' }); window.location.href = '/admin-login' }} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-red-400 hover:bg-red-500/10 transition ${!sidebarOpen ? 'justify-center' : ''}`}>
            <span className="text-lg">🚪</span>
      ...
      <main className={`flex-1 transition-all duration-300 ${sidebarOpen ? 'ml-56' : 'ml-16'}`}>
        <header className="sticky top-0 z-30 bg-[#f6f1ea]/95 backdrop-blur-xl border-b border-[#241a14]/10 px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            {pendingVerification.length > 0 && <button onClick={() => setActiveTab('orders')} className="text-xs bg-orange-100 text-orange-700 border border-orange-200 px-3 py-1.5 rounded-full font-semibold">🔔 {pendingVerification.length} Payment{pendingVerification.length > 1 ? 's' : ''} to Verify</button>}
            {pendingCancellations.length > 0 && <button onClick={() => setActiveTab('cancellations')} className="text-xs bg-red-100 text-red-700 border border-red-200 px-3 py-1.5 rounded-full font-semibold">❌ {pendingCancellations.length} Cancellation{pendingCancellations.length > 1 ? 's' : ''}</button>}
          </div>
        </header>
        ...
        <div className="p-6">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
              ...
              <div className="flex gap-3 flex-wrap">
                <div className="relative flex-1 min-w-48">
                  ...
                  <input type="text" placeholder="Search orders..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full rounded-full border border-[#241a14]/15 bg-white pl-10 pr-4 py-2.5 text-sm focus:outline-none focus:border-[#171313]/30 transition"/>
                </div>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="grid md:grid-cols-2 gap-6">
              <div className="rounded-[1.4rem] bg-white shadow-lg shadow-[#3d2619]/5 p-6">
                ...
                <form onSubmit={handleAddProduct} className="space-y-4">
                  <div><label className="text-sm text-[#7b6f66] mb-1 block">Name</label><input type="text" placeholder="Product name" value={productForm.name} onChange={(e) => setProductForm({ ...productForm, name: e.target.value })} className="w-full rounded-2xl border border-[#241a14]/15 bg-[#f6f1ea] px-4 py-3 text-sm focus:outline-none transition" required/></div>
                  <div><label className="text-sm text-[#7b6f66] mb-1 block">Description</label><textarea placeholder="Description" value={productForm.description} rows={3} onChange={(e) => setProductForm({ ...productForm, description: e.target.value })} className="w-full rounded-2xl border border-[#241a14]/15 bg-[#f6f1ea] px-4 py-3 text-sm focus:outline-none transition resize-none" required/></div>
                  <div className="grid grid-cols-2 gap-3">
                    <div><label className="text-sm text-[#7b6f66] mb-1 block">Price (₹)</label><input type="number" placeholder="999" value={productForm.price} onChange={(e) => setProductForm({ ...productForm, price: e.target.value })} className="w-full rounded-2xl border border-[#241a14]/15 bg-[#f6f1ea] px-4 py-3 text-sm focus:outline-none transition" required/></div>
                    <div><label className="text-sm text-[#7b6f66] mb-1 block">Original Price (₹)</label><input type="number" placeholder="1999" value={productForm.originalPrice} onChange={(e) => setProductForm({ ...productForm, originalPrice: e.target.value })} className="w-full rounded-2xl border border-[#241a14]/15 bg-[#f6f1ea] px-4 py-3 text-sm focus:outline-none transition" required/></div>
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <div><label className="text-sm text-[#7b6f66] mb-1 block">Category</label>
                      <select value={productForm.category} onChange={(e) => setProductForm({ ...productForm, category: e.target.value })} className="w-full rounded-2xl border border-[#241a14]/15 bg-[#f6f1ea] px-4 py-3 text-sm focus:outline-none transition">
                        {categories.length > 0 ? categories.filter(c => c.isActive).map(c => <option key={c.id} value={c.name}>{c.icon} {c.name}</option>) : ['Electronics', 'Fashion', 'Home', 'Beauty', 'Sports', 'Books', 'Toys'].map(c => <option key={c} value={c}>{c}</option>)}
                      </select>
                    </div>
                    <div><label className="text-sm text-[#7b6f66] mb-1 block">Stock</label><input type="number" placeholder="100" value={productForm.stock} onChange={(e) => setProductForm({ ...productForm, stock: e.target.value })} className="w-full rounded-2xl border border-[#241a14]/15 bg-[#f6f1ea] px-4 py-3 text-sm focus:outline-none transition" required/></div>
                  </div>
                  <div>
                    <label className="text-sm text-[#7b6f66] mb-2 block">Images</label>
                    {productForm.images.map((img, index) => (
                      <div key={index} className="flex gap-2 mb-2">
                        <input type="text" placeholder={`Image URL ${index + 1}`} value={img} onChange={(e) => { const u = [...productForm.images]; u[index] = e.target.value; setProductForm({ ...productForm, images: u }) }} className="flex-1 rounded-2xl border border-[#241a14]/15 bg-[#f6f1ea] px-4 py-2.5 text-sm focus:outline-none transition"/>
                  ...
                  <motion.button type="submit" whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} disabled={submittingProduct} className="w-full rounded-full bg-[#171313] py-3.5 text-sm font-semibold text-white transition hover:bg-[#3a2a21] disabled:opacity-50">{submittingProduct ? 'Adding...' : 'Add Product'}</motion.button>
                </form>
              </div>
              <div className="rounded-[1.4rem] bg-white shadow-lg shadow-[#3d2619]/5 p-6">
          ...
          {activeTab === 'cancellations' && (
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
              <div className="flex gap-3 flex-wrap">
                {[{ label: 'Pending', value: cancellations.filter(c => c.status === 'pending').length, color: 'text-amber-700 bg-amber-50 border-amber-200' }, { label: 'Approved', value: cancellations.filter(c => c.status === 'approved').length, color: 'text-green-700 bg-green-50 border-green-200' }, { label: 'Rejected', value: cancellations.filter(c => c.status === 'rejected').length, color: 'text-red-700 bg-red-50 border-red-200' }].map(s => (
                  <span key={s.label} className={`text-xs font-semibold px-3 py-1.5 rounded-full border ${s.color}`}>{s.label}: {s.value}</span>
                ))}
              </div>
              {cancellations.length === 0 ? (
                <div className="text-center py-20 rounded-[2rem] border border-[#241a14]/10 bg-white/55"><p className="text-4xl mb-4">✅</p><p className="text-[#7b6f66]">No cancellation requests!</p></div>
              ) : (
                <div className="space-y-4">
                  {cancellations.map((req, i) => (
                    <motion.div key={req.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }} className="rounded-[1.4rem] bg-white shadow-lg shadow-[#3d2619]/5 p-5">
                      <div className="flex items-start justify-between gap-4 flex-wrap mb-4">
                        ...
                        <span className={`text-xs font-semibold px-3 py-1 rounded-full border ${req.status === 'approved' ? 'text-green-700 bg-green-50 border-green-200' : req.status === 'rejected' ? 'text-red-700 bg-red-50 border-red-200' : 'text-amber-700 bg-amber-50 border-amber-200'}`}>{req.status.charAt(0).toUpperCase() + req.status.slice(1)}</span>
                      </div>
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-4">
                        <div className="rounded-2xl bg-[#f6f1ea] border border-[#241a14]/10 p-3"><p className="text-xs text-[#9b8f86] mb-1">Order Total</p><p className="font-semibold">₹{req.order?.total}</p></div>
                        <div className="rounded-2xl bg-[#f6f1ea] border border-[#241a14]/10 p-3"><p className="text-xs text-[#9b8f86] mb-1">Payment Method</p><p className="font-semibold text-sm">{paymentLabels[req.order?.paymentMethod] || 'N/A'}</p></div>
                        <div className="rounded-2xl bg-[#f6f1ea] border border-[#241a14]/10 p-3"><p className="text-xs text-[#9b8f86] mb-1">Order Status</p><p className={`font-semibold text-sm ${req.order?.status === 'cancelled' ? 'text-red-600' : 'text-[#171313]'}`}>{req.order?.status?.charAt(0).toUpperCase() + req.order?.status?.slice(1)}</p></div>
                      </div>
                      ...
                      {req.status === 'pending' && (
                        <div className="flex gap-3">
                          <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => handleCancellation(req.id, 'approve')} disabled={updating === req.id + '_approve'} className="flex-1 rounded-full bg-red-600 py-2.5 text-sm font-semibold text-white transition hover:bg-red-700 disabled:opacity-50">{updating === req.id + '_approve' ? '...' : '✅ Approve Cancellation'}</motion.button>
                          <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => handleCancellation(req.id, 'reject')} disabled={updating === req.id + '_reject'} className="flex-1 rounded-full border border-[#241a14]/15 py-2.5 text-sm font-semibold text-[#6d625a] transition hover:bg-[#f6f1ea] disabled:opacity-50">{updating === req.id + '_reject' ? '...' : '❌ Reject Request'}</motion.button>
                        </div>
                      )}
                    </motion.div>
                  ))}
                        ...
                        <button onClick={() => setExpandedTicket(isExpanded ? null : ticket.id)} className="w-full flex items-center justify-between gap-4 p-5 text-left hover:bg-[#f9f6f2] transition">
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-2 flex-wrap mt-0.5">
                              <p className="text-sm text-[#7b6f66]">{ticket.name} · {ticket.email}</p>
                              {ticket.whatsapp && <p className="text-xs text-green-600 font-medium">💬 {ticket.whatsapp}</p>}
                              {ticket.isGuest
                                ? <span className="text-xs px-2 py-0.5 rounded-full bg-amber-50 border border-amber-200 text-amber-700 font-semibold">👤 Guest</span>
                                : <span className="text-xs px-2 py-0.5 rounded-full bg-green-50 border border-green-200 text-green-700 font-semibold">✓ Registered</span>
                              }
                            </div>
                          ...
                          {isExpanded && (
                            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden border-t border-[#241a14]/10">
                              ...
                              <div className="p-4 border-t border-[#241a14]/10 bg-white">
                                ...
                                <div className="flex gap-2">
                                  <textarea placeholder="Type reply..." value={replyForm[ticket.id] || ''} onChange={(e) => setReplyForm(prev => ({ ...prev, [ticket.id]: e.target.value }))} rows={2} className="flex-1 rounded-2xl border border-[#241a14]/15 bg-[#f6f1ea] px-4 py-2.5 text-sm focus:outline-none transition resize-none" onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleTicketReply(ticket.id) } }}/>
                                  <div className="flex flex-col gap-2">
                                    <motion.button whileHover={{ scale: 1.05 }} onClick={() => handleTicketReply(ticket.id)} disabled={sendingReply === ticket.id || !replyForm[ticket.id]?.trim()} className="rounded-full bg-[#171313] px-4 py-2 text-xs font-semibold text-white disabled:opacity-50">{sendingReply === ticket.id ? '...' : 'Send'}</motion.button>
                                    <button onClick={() => handleTicketResolve(ticket.id, ticket.status === 'resolved' ? 'open' : 'resolved')} className={`rounded-full px-3 py-1.5 text-xs font-semibold transition ${ticket.status === 'resolved' ? 'border border-amber-200 bg-amber-50 text-amber-700' : 'bg-green-600 text-white hover:bg-green-700'}`}>{ticket.status === 'resolved' ? 'Reopen' : 'Resolve'}</button>
                                  </div>
                                </div>
                              </div>
                            </motion.div>
                    ...
                    <div className="rounded-[1.4rem] bg-white shadow-lg shadow-[#3d2619]/5 p-6">
                      ...
                      <form onSubmit={handleCouponSubmit} className="space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div><label className="text-sm text-[#7b6f66] mb-1 block">Code *</label><input type="text" placeholder="SAVE10" value={couponForm.code} onChange={(e) => setCouponForm({ ...couponForm, code: e.target.value.toUpperCase() })} disabled={!!editCoupon} className="w-full rounded-2xl border border-[#241a14]/15 bg-[#f6f1ea] px-4 py-3 text-sm uppercase font-mono font-bold focus:outline-none transition disabled:opacity-50" required/></div>
                          <div><label className="text-sm text-[#7b6f66] mb-1 block">Description</label><input type="text" placeholder="10% off for new users" value={couponForm.description} onChange={(e) => setCouponForm({ ...couponForm, description: e.target.value })} className="w-full rounded-2xl border border-[#241a14]/15 bg-[#f6f1ea] px-4 py-3 text-sm focus:outline-none transition"/></div>
                          ...
                          <div><label className="text-sm text-[#7b6f66] mb-1 block">Value *</label><input type="number" placeholder={couponForm.type === 'percentage' ? '10' : '50'} value={couponForm.value} onChange={(e) => setCouponForm({ ...couponForm, value: e.target.value })} className="w-full rounded-2xl border border-[#241a14]/15 bg-[#f6f1ea] px-4 py-3 text-sm focus:outline-none transition" required/></div>
                          {couponForm.type === 'percentage' && <div><label className="text-sm text-[#7b6f66] mb-1 block">Max Discount (₹)</label><input type="number" placeholder="150" value={couponForm.maxDiscount} onChange={(e) => setCouponForm({ ...couponForm, maxDiscount: e.target.value })} className="w-full rounded-2xl border border-[#241a14]/15 bg-[#f6f1ea] px-4 py-3 text-sm focus:outline-none transition"/></div>}
                          <div><label className="text-sm text-[#7b6f66] mb-1 block">Min Cart Value (₹)</label><input type="number" placeholder="799" value={couponForm.minCartValue} onChange={(e) => setCouponForm({ ...couponForm, minCartValue: e.target.value })} className="w-full rounded-2xl border border-[#241a14]/15 bg-[#f6f1ea] px-4 py-3 text-sm focus:outline-none transition"/></div>
                          ...
                          {couponForm.usageType === 'total_limit' && <div><label className="text-sm text-[#7b6f66] mb-1 block">Total Limit</label><input type="number" placeholder="500" value={couponForm.totalLimit} onChange={(e) => setCouponForm({ ...couponForm, totalLimit: e.target.value })} className="w-full rounded-2xl border border-[#241a14]/15 bg-[#f6f1ea] px-4 py-3 text-sm focus:outline-none transition"/></div>}
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-3xl space-y-6">
              <div className="rounded-[1.4rem] bg-white shadow-lg shadow-[#3d2619]/5 p-6">
                ...
                <form onSubmit={handleAddCategory} className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div><label className="text-sm text-[#7b6f66] mb-1 block">Category Name *</label><input type="text" placeholder="e.g. Gaming, Furniture..." value={categoryForm.name} onChange={(e) => setCategoryForm(prev => ({ ...prev, name: e.target.value }))} className="w-full rounded-2xl border border-[#241a14]/15 bg-[#f6f1ea] px-4 py-3 text-sm focus:outline-none focus:border-[#171313]/30 transition" required/></div>
                    <div><label className="text-sm text-[#7b6f66] mb-1 block">Emoji Icon</label><input type="text" placeholder="🛍️" value={categoryForm.icon} onChange={(e) => setCategoryForm(prev => ({ ...prev, icon: e.target.value }))} className="w-full rounded-2xl border border-[#241a14]/15 bg-[#f6f1ea] px-4 py-3 text-sm focus:outline-none focus:border-[#171313]/30 transition"/></div>
                    <div className="md:col-span-2">
                      ...
                      <input type="text" placeholder="https://res.cloudinary.com/..." value={categoryForm.image} onChange={(e) => setCategoryForm(prev => ({ ...prev, image: e.target.value }))} className="w-full rounded-2xl border border-[#241a14]/15 bg-[#f6f1ea] px-4 py-3 text-sm focus:outline-none focus:border-[#171313]/30 transition"/>
                      {categoryForm.image && <img src={categoryForm.image} alt="preview" className="mt-2 h-20 w-20 rounded-2xl object-cover border border-[#241a14]/10"/>}
                    </div>
                  </div>
                  {categoryForm.name && (
                    <div className="flex items-center gap-3 bg-[#f6f1ea] rounded-2xl p-4">
                      <span className="text-3xl">{categoryForm.icon || '🛍️'}</span>
                      <div><p className="text-xs text-[#9b8f86]">Preview</p><p className="font-semibold">{categoryForm.name}</p></div>
                    </div>
                  )}
                  {categoryMessage && <p className={`text-sm ${categoryMessage.includes('!') && !categoryMessage.includes('Cannot') && !categoryMessage.includes('already') ? 'text-green-600' : 'text-red-500'}`}>{categoryMessage}</p>}
                  <motion.button type="submit" whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} disabled={submittingCategory || !categoryForm.name.trim()} className="w-full rounded-full bg-[#171313] py-3.5 text-sm font-semibold text-white transition hover:bg-[#3a2a21] disabled:opacity-50">{submittingCategory ? 'Adding...' : '+ Add Category'}</motion.button>
                </form>
              </div>
              <div className="rounded-[1.4rem] bg-white shadow-lg shadow-[#3d2619]/5 p-6">
                ...
                {categories.length === 0 ? (
                  <div className="text-center py-10"><p className="text-4xl mb-3">🗂️</p><p className="text-sm text-[#7b6f66]">No categories yet</p></div>
                ) : (
                  <div className="space-y-3">
                      ...
                      <div key={category.id} className={`rounded-2xl border-2 p-4 transition ${category.isActive ? 'border-[#241a14]/10 bg-[#f6f1ea]' : 'border-[#241a14]/5 bg-[#f6f1ea]/50 opacity-60'}`}>
                        {editCategory?.id === category.id ? (
                          <div className="flex items-center gap-3 flex-wrap">
                            <input type="text" value={editCategory.icon} onChange={(e) => setEditCategory(prev => ({ ...prev, icon: e.target.value }))} className="w-16 rounded-xl border border-[#241a14]/15 bg-white px-3 py-2 text-sm text-center focus:outline-none" placeholder="Icon"/>
                            <input type="text" value={editCategory.name} onChange={(e) => setEditCategory(prev => ({ ...prev, name: e.target.value }))} className="flex-1 rounded-xl border border-[#241a14]/15 bg-white px-3 py-2 text-sm focus:outline-none" placeholder="Category name"/>
                            <input type="text" value={editCategory.image || ''} onChange={(e) => setEditCategory(prev => ({ ...prev, image: e.target.value }))} className="flex-1 rounded-xl border border-[#241a14]/15 bg-white px-3 py-2 text-sm focus:outline-none" placeholder="Image URL (Cloudinary)"/>
                            <div className="flex gap-2 flex-shrink-0">
                              <button onClick={() => handleUpdateCategory(category)} className="rounded-full bg-[#171313] px-4 py-2 text-xs font-semibold text-white transition hover:bg-[#3a2a21]">Save</button>
                              <button onClick={() => setEditCategory(null)} className="rounded-full border border-[#241a14]/15 px-4 py-2 text-xs font-semibold text-[#6d625a] transition hover:bg-white">Cancel</button>
                            </div>
                          </div>
                        ) : (
                          <div className="flex items-center justify-between gap-4">
                            <div className="flex items-center gap-3 flex-1 min-w-0">
                              {category.image ? <img src={category.image} alt={category.name} className="w-10 h-10 rounded-xl object-cover flex-shrink-0"/> : <span className="text-2xl flex-shrink-0">{category.icon}</span>}
                              <div className="min-w-0">
                                <p className="font-semibold text-sm">{category.name}</p>
                                <p className="text-xs text-[#9b8f86] mt-0.5">{category.isActive ? '✓ Active in store' : '✕ Hidden from store'}</p>
                              </div>
                            </div>
                            <div className="flex items-center gap-3 flex-shrink-0">
                              <button onClick={() => handleToggleCategory(category)} className={`w-11 h-6 rounded-full transition-colors duration-200 relative ${category.isActive ? 'bg-[#171313]' : 'bg-[#241a14]/20'}`}><span className={`absolute top-1 w-4 h-4 rounded-full bg-white transition-all duration-200 ${category.isActive ? 'left-6' : 'left-1'}`}/></button>
                              <button onClick={() => setEditCategory({ id: category.id, name: category.name, icon: category.icon, image: category.image || '' })} className="text-xs px-3 py-1.5 rounded-full border border-[#241a14]/15 text-[#6d625a] hover:bg-white transition">Edit</button>
                              <button onClick={() => handleDeleteCategory(category.id)} className="text-xs px-3 py-1.5 rounded-full border border-red-200 text-red-500 hover:bg-red-50 transition">Delete</button>
                            </div>
                          </div>
                        )}
                      </div>
          ...
          {activeTab === 'users' && (
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
              <div className="flex items-center justify-between flex-wrap gap-3">
                <p className="text-sm text-[#7b6f66]">{users.length} registered users</p>
                <div className="relative">
                  <span className="absolute left-4 top-1/2 -translate-y-1/2 text-[#9b8f86]">🔍</span>
                  <input type="text" placeholder="Search users..." value={userSearch} onChange={(e) => setUserSearch(e.target.value)} className="rounded-full border border-[#241a14]/15 bg-white pl-10 pr-4 py-2.5 text-sm focus:outline-none focus:border-[#171313]/30 transition w-64"/>
                </div>
              </div>
              {filteredUsers.length === 0 ? (
                <div className="text-center py-20 rounded-[2rem] border border-[#241a14]/10 bg-white/55"><p className="text-4xl mb-4">👤</p><p className="text-[#7b6f66]">No users found</p></div>
              ) : (
                <div className="space-y-4">
                  {filteredUsers.map(user => (
                    <div key={user.id} className="rounded-[1.4rem] bg-white shadow-lg shadow-[#3d2619]/5 p-5">
                      <div className="flex items-start justify-between gap-4 flex-wrap mb-4">
                        <div className="flex items-center gap-4">
                          <div className="w-12 h-12 rounded-full bg-[#171313] text-white flex items-center justify-center text-lg font-semibold flex-shrink-0">{user.name?.charAt(0).toUpperCase()}</div>
                          <div>
                            <p className="font-semibold">{user.name}</p>
                            <p className="text-sm text-[#7b6f66]">{user.email}</p>
                            <p className="text-xs text-[#9b8f86] mt-0.5">Joined {new Date(user.createdAt).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}</p>
                          </div>
                        </div>
                        <span className={`text-xs font-semibold px-3 py-1 rounded-full border flex-shrink-0 ${user.verified ? 'text-green-700 bg-green-50 border-green-200' : 'text-amber-700 bg-amber-50 border-amber-200'}`}>{user.verified ? '✓ Verified' : '⏳ Unverified'}</span>
                      </div>
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-3">
                        {[
                          ...
                          { label: 'Date of Birth', value: user.dob ? new Date(user.dob).toLocaleDateString('en-IN') : '—' }
                        ].map((item, i) => (
                          <div key={i} className="rounded-2xl bg-[#f6f1ea] border border-[#241a14]/10 px-3 py-2">
                            <p className="text-xs text-[#9b8f86]">{item.label}</p>
                            <p className="text-sm font-semibold mt-0.5 truncate">{item.value}</p>
                          </div>
                        ))}
                      </div>
                      {(user.address || user.pincode) && (
                        <div className="rounded-2xl bg-[#f6f1ea] border border-[#241a14]/10 px-3 py-2">
                          <p className="text-xs text-[#9b8f86]">📍 Address</p>
                          <p className="text-sm font-semibold mt-0.5">{[user.address, user.pincode].filter(Boolean).join(', ')}</p>
                        </div>
                      )}
                    </div>
                  ))}
          ...
          {activeTab === 'admins' && (
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
              <div className="flex justify-end">
                <button onClick={() => { setShowAdminForm(!showAdminForm); setEditAdmin(null); setAdminForm({ name: '', email: '', password: '', permissions: [], isActive: true }) }} className="rounded-full bg-[#171313] px-6 py-3 text-sm font-semibold text-white transition hover:bg-[#3a2a21]">{showAdminForm ? '✕ Cancel' : '+ Add Admin'}</button>
              </div>
              <div className="rounded-2xl bg-blue-50 border border-blue-200 p-4">
                <p className="text-sm text-blue-700 font-semibold mb-1">ℹ️ Sub-admin Login</p>
                <p className="text-xs text-blue-600">Sub-admins login at <span className="font-mono font-bold">/staff-login</span></p>
              </div>
              <AnimatePresence>
                {showAdminForm && (
                  <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden">
                    <div className="rounded-[1.4rem] bg-white shadow-lg shadow-[#3d2619]/5 p-6">
                      ...
                      <form onSubmit={handleAdminSubmit} className="space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div><label className="text-sm text-[#7b6f66] mb-1 block">Full Name *</label><input type="text" placeholder="Rahul Sharma" value={adminForm.name} onChange={(e) => setAdminForm({ ...adminForm, name: e.target.value })} className="w-full rounded-2xl border border-[#241a14]/15 bg-[#f6f1ea] px-4 py-3 text-sm focus:outline-none transition" required/></div>
                          <div><label className="text-sm text-[#7b6f66] mb-1 block">Email *</label><input type="email" placeholder="admin@example.com" value={adminForm.email} onChange={(e) => setAdminForm({ ...adminForm, email: e.target.value })} disabled={!!editAdmin} className="w-full rounded-2xl border border-[#241a14]/15 bg-[#f6f1ea] px-4 py-3 text-sm focus:outline-none transition disabled:opacity-50" required={!editAdmin}/></div>
                          <div className="md:col-span-2"><label className="text-sm text-[#7b6f66] mb-1 block">{editAdmin ? 'New Password (leave blank to keep)' : 'Password *'}</label><input type="password" placeholder="••••••••" value={adminForm.password} onChange={(e) => setAdminForm({ ...adminForm, password: e.target.value })} className="w-full rounded-2xl border border-[#241a14]/15 bg-[#f6f1ea] px-4 py-3 text-sm focus:outline-none transition" required={!editAdmin}/></div>
                        </div>
                        <div>
              ...
              <div className="rounded-[1.4rem] bg-white shadow-lg shadow-[#3d2619]/5 p-6">
                ...
                <div className="space-y-3">
                  {[
                    ...
                    { id: 'card', icon: '💳', label: 'Razorpay', desc: 'Card, UPI, Wallet via Razorpay' }
                  ].map(option => (
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
              ...
              <div className="rounded-[1.4rem] bg-white shadow-lg shadow-[#3d2619]/5 p-6">
                ...
                {products.filter(p => p.stock <= 5).length === 0 ? (
                  <div className="text-center py-6"><p className="text-3xl mb-2">✅</p><p className="text-sm text-[#7b6f66]">All products are well stocked!</p></div>
                ) : (
                  <div className="space-y-3">
                    {products.filter(p => p.stock <= 5).sort((a, b) => a.stock - b.stock).map(product => (
                      <div key={product.id} className={`flex items-center justify-between p-3 rounded-2xl border ${product.stock === 0 ? 'bg-red-50 border-red-200' : 'bg-amber-50 border-amber-200'}`}>
                        ...
                        <span className={`text-xs font-bold px-3 py-1 rounded-full ${product.stock === 0 ? 'bg-red-100 text-red-700' : 'bg-amber-100 text-amber-700'}`}>{product.stock === 0 ? '🚫 Out of Stock' : `⚠️ ${product.stock} left`}</span>
                      </div>
                    ))}
                  </div>
                )}
                  ...
                  {orders.slice(0, 8).map(order => (
                    <div key={order.id} className="flex items-center justify-between gap-4 p-3 rounded-2xl bg-[#f6f1ea]">
                      <div className="flex items-center gap-3 flex-wrap flex-1 min-w-0">
                        <div><p className="text-xs text-[#9b8f86]">Order ID</p><p className="text-sm font-mono font-bold">{order.orderId}</p></div>
                        <div><p className="text-xs text-[#9b8f86]">Customer</p><p className="text-sm">{order.name}</p></div>
                        {order.isSameDay && <span className="text-xs font-bold text-blue-600 bg-blue-50 border border-blue-200 px-2 py-0.5 rounded-full">⚡ Same Day</span>}
                      </div>
                      <div className="flex items-center gap-2 flex-shrink-0">
                        <span className="text-sm font-semibold">₹{order.total}</span>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={(e) => { if (e.target === e.currentTarget) setViewScreenshot(null) }}>
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-white rounded-[1.4rem] w-full max-w-lg overflow-hidden shadow-2xl">
              <div className="bg-[#171313] p-5 flex items-center justify-between">
                <div><p className="text-white font-semibold">Payment Screenshot</p><p className="text-white/60 text-xs mt-0.5">{viewScreenshot.orderId} — {viewScreenshot.name}</p></div>
                <button onClick={() => setViewScreenshot(null)} className="text-white/60 hover:text-white text-xl transition">✕</button>
              </div>
              <div className="p-5">
                <div className="rounded-2xl overflow-hidden border-2 border-[#241a14]/10 mb-5"><img src={viewScreenshot.paymentScreenshot} alt="Payment Screenshot" className="w-full object-contain max-h-96 bg-[#f6f1ea]"/></div>
                <div className="grid grid-cols-2 gap-3 mb-5">
                  {viewScreenshot.paymentSenderName && <div className="rounded-2xl bg-[#f6f1ea] border border-[#241a14]/10 p-4"><p className="text-xs text-[#9b8f86] mb-1 uppercase tracking-wider">Sender Name</p><p className="text-sm font-bold">{viewScreenshot.paymentSenderName}</p></div>}
                  {viewScreenshot.paymentUTR && <div className="rounded-2xl bg-[#f6f1ea] border border-[#241a14]/10 p-4"><p className="text-xs text-[#9b8f86] mb-1 uppercase tracking-wider">UTR / Transaction</p><p className="text-sm font-bold font-mono">{viewScreenshot.paymentUTR}</p></div>}
                </div>
                <div className="rounded-2xl bg-[#f6f1ea] border border-[#241a14]/10 p-4 mb-5">
                  <p className="text-xs text-[#9b8f86] mb-1 uppercase tracking-wider">Order ID</p>
                  <p className="text-sm font-bold font-mono">{viewScreenshot.orderId}</p>
                ...
                <div className="flex gap-3">
                  ...
                  {viewScreenshot.paymentVerified && <div className="flex-1 rounded-full bg-green-50 border-2 border-green-200 py-3 text-sm font-semibold text-green-700 text-center">✓ Already Verified</div>}
                </div>
              </div>
            </motion.div>
          </motion.div>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  {editForm.images.map((img, index) => (
                    <div key={index} className="flex gap-2 mb-2">
                      <input type="text" placeholder={`Image URL ${index + 1}`} value={img} onChange={(e) => { const u = [...editForm.images]; u[index] = e.target.value; setEditForm({ ...editForm, images: u }) }} className="flex-1 rounded-2xl border border-[#241a14]/15 bg-[#f6f1ea] px-4 py-2.5 text-sm focus:outline-none transition"/>
                ...
                <div>
                  ...
                  <div className="flex gap-2 mb-2">
                    <input type="text" placeholder="6-digit pincode" value={editPincodeInput} onChange={(e) => setEditPincodeInput(e.target.value.replace(/\D/g, '').slice(0, 6))} onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); addPincode(editForm.sameDayPincodes, (p) => setEditForm({ ...editForm, sameDayPincodes: p }), editPincodeInput, setEditPincodeInput) } }} className="flex-1 rounded-2xl border border-[#241a14]/15 bg-[#f6f1ea] px-4 py-2.5 text-sm focus:outline-none transition"/>
                ...
                <div className="flex gap-3 pt-2">
                  ...
                  <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={handleUpdateProduct} disabled={updating === 'product'} className="flex-1 rounded-full bg-[#171313] py-3 text-sm font-semibold text-white transition hover:bg-[#3a2a21] disabled:opacity-50">{updating === 'product' ? 'Saving...' : 'Save Changes'}</motion.button>
                </div>
              </div>
            </motion.div>
          </motion.div>
