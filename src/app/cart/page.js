  ...
  return (
    <main className="min-h-screen bg-[#f6f1ea] text-[#171313]">
      ...
      <div className="mx-auto max-w-6xl px-5 py-10">
        ...
        {cart.length === 0 ? (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-24 rounded-[2rem] border border-[#241a14]/10 bg-white/55">
            ...
            <div className="md:col-span-2 space-y-4">
              {cart.map((item, i) => (
                <motion.div
                  ...
                  className="overflow-hidden rounded-[1.4rem] bg-white shadow-lg shadow-[#3d2619]/5 p-4 flex gap-4"
                >
                  <div className="w-20 h-20 rounded-2xl overflow-hidden flex-shrink-0 bg-[#eadfd4]">
                    {item.images?.[0] ? (
                  ...
                  <div className="flex flex-col items-end justify-between">
                    <button onClick={() => removeItem(item.id)} className="text-sm text-[#9b8f86] hover:text-red-500 transition">✕</button>
                    <div className="flex items-center gap-2 rounded-full border border-[#241a14]/10 bg-[#f6f1ea] px-3 py-1">
                      <button onClick={() => updateQuantity(item.id, item.quantity - 1)} className="font-semibold text-[#7b6f66] hover:text-[#171313]">−</button>
              ...
              <button onClick={clearCart} className="text-sm text-red-400 hover:text-red-600 transition">Clear Cart</button>
            </div>

            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="overflow-hidden rounded-[1.4rem] bg-white shadow-lg shadow-[#3d2619]/5 p-6 h-fit sticky top-24">
